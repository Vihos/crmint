...
def execute_command(step_name, command, cwd='.', report_empty_err=True, debug=False, stream_output_in_debug=True,
                    force_std_out=False, disable_spinner=False, allowed_exit_codes=None):
    ...
    with spinner.spinner(disable=debug or disable_spinner or force_std_out, color='blue', bold=True):

        if disable_spinner and not debug:
            click.echo("\n", nl=False)
        ...
        try:
            ...
            # Pass kill signal to process and wait until shut down of process
            pipe.send_signal(signal.SIGINT)
            out, err = pipe.communicate()

            raise KeyboardInterrupt(e)

    if not debug and not disable_spinner:
        click.echo("\n", nl=False)
    if debug and not stream_output_in_debug:
        click.echo(out)
    if pipe.returncode not in allowed_exit_codes and err and (len(err) > 0 or report_empty_err):
...
def get_default_stage_name(debug=False):
    ...
    status, out, err = execute_command("Get current project identifier", command, debug=debug,
