...
@init.command('backend')
@click.option('--debug/--no-debug', default=False)
def init_frontend(debug):
    """Initialize backend for local development."""
...
@init.command('frontend')
@click.option('--debug/--no-debug', default=False)
def init_frontend(debug):
    """Initialize frontend for local development."""
    click.echo(click.style(">>>> Initialization", fg='magenta', bold=True))
...
def init_database():
    ...
    components = [
        (
            "Create database",
            "echo \"CREATE DATABASE IF NOT EXISTS {db_name} CHARACTER SET utf8;\" | sudo mysql -u root".format(
                **params),
        ),
        (
            "Grant privileges to {db_user}".format(**params),
            "echo \"GRANT ALL PRIVILEGES ON {db_user}.* TO '{db_user}'@'localhost' IDENTIFIED BY '{db_pass}';\" | sudo mysql -u root".format(
                **params),
        ),
        (
            "Cleaning",
            "echo \"FLUSH PRIVILEGES;\" | sudo mysql -u root".format(**params),
        ),
        (
